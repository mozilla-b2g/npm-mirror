...
SyncManager.prototype = {
      ...
      SyncManager.download(packageVersionUrl, function(e, packageVersionData) {
        ...
        this.syncPackageDeps(packageName, version, packageVersion, function() {
          this.savePackageVersion(
            packageName, version, packageVersion.dependencies,
            packageVersion.devDependencies, function() {
  ...
  syncPackageDeps: function(packageName, version, packageVersion, callback) {
    ...
    if (packageVersion.dependencies) {
      Object.keys(packageVersion.dependencies).forEach(function(dep) {
        dependencies.push(dep);
      });
    }

    if (packageVersion.devDependencies) {
      Object.keys(packageVersion.devDependencies).forEach(function(dep) {
        dependencies.push(dep);
      });
    }
   ...
   * @param {Object} devDependencies map from devDependency name to version.
   * @param {Function} callback invoke when done.
   */
  savePackageVersion: function(packageName, version, dependencies,
    devDependencies, callback) {
    var packageVersion = {};
    packageVersion.name = packageName;
    packageVersion.version = version;
    packageVersion.dependencies = dependencies;
    packageVersion.devDependencies = devDependencies;
    packageVersion.dist = {
