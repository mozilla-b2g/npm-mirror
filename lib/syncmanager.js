...
SyncManager.prototype = {
  ...
  savePackageRoot: function(packageName, versions, callback) {
    ...
    versions.forEach(function(version) {
      packageRoot.versions[version] = this.getPackageVersionUrl(packageName, version);
    }.bind(this));
      ...
      SyncManager.download(packageVersionUrl, function(e, packageVersionData) {
        ...
        this.syncPackageDeps(packageName, version, packageVersion, function() {
          this.savePackageVersion(
            packageName, version, packageVersion.dependencies, function() {
  ...
  syncPackageDeps: function(packageName, version, packageVersion, callback) {
    ...
    var dependencies = Object.keys(packageVersion.dependencies);
   ...
   * @param {string} packageName name of the package.
   * @param {string} version package version.
   * @param {Object} dependencies map from dependency name to version.
   * @param {Function} callback invoke when done.
   */
  savePackageVersion: function(packageName, version, dependencies, callback) {
    var packageVersion = {};
    packageVersion.name = packageName;
    packageVersion.version = version;
    packageVersion.dependencies = dependencies;
    packageVersion.dist = {
    ...
    fs.writeFile(filename, JSON.stringify(packageVersion), callback);
  },

  /**
   * Map a package name and version to a package version url.
   *
   * @param {string} packageName name of the package.
   * @param {string} version package version.
   * @return {string} package version url.
   */
  getPackageVersionUrl: function(packageName, version) {
    return url.resolve(this.host, packageName + '/' + version);
  },
